...
Small models are only interesting if training them is cheap and the numbers are real. The flagship experiment is issue triage: 8,332 real Kubernetes issues, labels `bug` / `docs` / `feature` / `support`, split 5,832 train / 1,250 calibration / 1,250 test with a fixed stratified seed. The split is random, not temporal, and duplicate leakage was not audited — so read these as "same-distribution" numbers.

| setup | labels used | train time | test accuracy | macro-F1 | per question |
|---|---|---|---|---|---|
| TF-IDF + logreg (full) | 5,832 | 13.1 s | 82.72% | 0.7875 | ~1.1 ms |
| frozen MiniLM + logreg | 5,832 | 788 s encode + 0.6 s fit | 78.16% | 0.7335 | ~96.5 ms |
| SetFit, 32 per class | 128 | 1,064 s | 72.88% | 0.6839 | ~176 ms |
| TF-IDF, 32 per class | 128 | 0.9 s | 68.48% | 0.6359 | ~0.002 ms |
| SetFit, 16 per class | 64 | 536 s | 63.92% | 0.5735 | ~178 ms |
| TF-IDF, 16 per class | 64 | 0.8 s | 62.48% | 0.5619 | ~0.002 ms |
| SetFit, 8 per class | 32 | 297 s | 59.28% | 0.5498 | ~180 ms |
| TF-IDF, 8 per class | 32 | 0.8 s | 66.16% | 0.6202 | ~0.002 ms |

SetFit here is few-shot contrastive fine-tuning of `all-MiniLM-L6-v2` (1 epoch, batch 16). The TF-IDF rows use the exact same 8/16/32-per-class subsamples, so the comparison is about label efficiency, not data. Calibration (temperature scaling) was fitted on the calibration split only, for every row.

![What tens of labels buy you](benches/charts/fewshot_accuracy.png)
![What those extra points cost](benches/charts/fewshot_cost.png)
![The price you pay at decision time](benches/charts/fewshot_latency.png)

Reading it straight:

- **With tens of labels, the encoder starts earning its keep — eventually.** SetFit trails same-budget TF-IDF at 8 examples per class (59.3 vs 66.2) but overtakes it by 32 (72.9 vs 68.5). The crossover is real, and the price is steep: roughly 1,000× the training time and 100,000× the per-question latency for +4.4 points. Whether that trade is worth it depends on how much a labeled example costs you.
- **Embeddings help; they don't beat a well-tuned baseline.** Frozen MiniLM embeddings + logistic regression on all 5,832 examples reach 78.2% — better than any few-shot run, still 4.6 points behind full-data TF-IDF at 82.7%. On this task the humble bag-of-words wins, and it trains in 13 seconds.
- **Calibration behaves everywhere.** Temperature scaling on the held-out calibration set cut expected calibration error from ~0.19–0.32 down to ~0.04–0.05 across every few-shot row, with accuracy untouched — the theory holding up on real data.

Caveats, attached: the few-shot rows are a single draw (seed 42) — few-shot sampling is noisy and a different seed could move those numbers several points, so don't treat any single figure as the method's true mean. `num_iterations=5` is a cost-driven deviation from SetFit's default of 20; more iterations might widen SetFit's lead, at ~14 s/step on 2 CPU threads. Wall clocks are from a 2-thread CPU shared between runs — relative ordering is robust, exact seconds are not. Latency is encode+predict averaged over the test questions; TF-IDF rows are scikit-learn in-process while SetFit/frozen are encoder-bound, so the ~100,000× inference gap partly reflects implementation, not just architecture.

This is the evidence behind the `gavel train` ladder on the roadmap: an instant TF-IDF baseline in seconds, a SetFit few-shot rung for when tens of labels exist and minutes of training are acceptable, and teacher distillation for the zero-label case.

### Against a hosted generalist

Same 1,250-issue test set, measured 2026-09-20: gavel's trained specialist (TF-IDF engine with energy abstention) against a hosted zero-shot decision API.

| | Gavel (trained) | Hosted API (zero-shot) |
|---|---|---|
| Accuracy, answered only | 84.30% (1004/1191) | 81.95% (849/1036) |
| Accuracy, abstentions/errors count as wrong | 80.32% | 67.92% |
| Macro-F1 | 0.8046 | 0.7902 |
| ECE | 0.0306 | 0.0897 |
| Abstention rate | 4.72% | — |
| Request errors | 0 | 17.12% (provider-side drops) |
| Latency p50 / p99 | 1.1 / 6.7 ms | 436.1 / 732.9 ms |
| Marginal cost per 1,000 decisions | ~$0 (owned CPU, hardware excluded) | $0.0401 |

Say it plainly: this is a trained specialist against a zero-shot generalist — an expected win, not a verdict on anyone's intelligence, and not a claim that gavel is categorically better. Gavel ran locally; the hosted side includes network and provider-side failures. The defensible edges are +2.35 points answered accuracy, +12.4 points counting failures as wrong, and ~400× lower p50 latency measured locally. The hosted API's real advantage is the one gavel doesn't have yet: it needed zero training data, which is exactly the gap `gavel train` has to close.
...
- [Tetris, one typed decision per piece](docs/cookbooks/tetris-typed-decisions.md) — second capability demo, same pattern: a heuristic shortlists two landings, one `/ask` call picks the cleaner one (0.3 s train, 98.4% val, 0.2 ms/decision, ~675 pieces/s), with disclosed heuristic fallbacks and live CPU/GPU load on screen.
- Generalist program (board entry track): pairwise scorer head over frozen Qwen3-4B — 90.5% item accuracy on public JevBench-style decisions (easy 95.8%, hard 86.2%), temperature-calibrated, verified by fresh download. Weights + eval: [Hugging Face](https://huggingface.co/syedsohailhussain/gavel-decide-4b) — code on the `generalist-v1` branch.
